...
def td_prediction(env, policy_probs, alpha=0.1, gamma=1.0,
    ...
    for ep in range(n_episodes):
        ...
        while not done and step < 200:
            ...
            if done or next_state == -1:
                td_target = reward          # 终止状态 V=0
            else:
                td_target = reward + gamma * V[next_state]

            td_error = td_target - V[state]  # δ_t
            V[state] += alpha * td_error     # 增量更新
        ...
        if verbose and (ep + 1) % 200 == 0:
            ...
            print(f"  回合 {ep+1:>4d}/{n_episodes} | 最近200步平均|TD误差|: {mean_error:.4f}")

    return V, errors


def td_prediction_with_decay(env, policy_probs, gamma=1.0,
                             n_episodes=1000, verbose=True):
    """
    带学习率衰减的 TD(0)。
    α_t = 1 / (1 + t/N(s))，访问越多的状态更新幅度越小。
    """
    V = np.zeros(env.n_states)
    N = np.zeros(env.n_states)  # 每个状态的访问计数
